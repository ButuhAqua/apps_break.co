...
class _InventoryGerobakBPageState extends State<InventoryGerobakBPage> {
  ...
  Future<void> _loadInventory() async {
    ...
    final getProductInventoryByLocation = GetProductInventoryByLocation(repository);

    setState(() {
      _futureInventory = getProductInventoryByLocation(
        'Gerobak B', // ganti Gerobak A / Gerobak B sesuai file
        token,
      );
    });
  }
            ...
            child: ListView(
              ...
              children: [
                _chip(
                  ...
                  onTap: () => setState(() => _statusFilter = 'Semua'),
                ),
                const SizedBox(width: 8),
                _chip(
                  'OK',
                  selected: _statusFilter == 'OK',
                  onTap: () => setState(() => _statusFilter = 'OK'),
                ),
                const SizedBox(width: 8),
                _chip(
                  'Low',
                ...
                _chip(
                  'Habis',
                  selected: _statusFilter == 'Habis',
                  onTap: () => setState(() => _statusFilter = 'Habis'),
                ),
            ...
            child: FutureBuilder<List<ProductInventoryItem>>(
              ...
              builder: (context, snapshot) {
                ...
                if (filteredData.isEmpty) {
                  return const Center(
                    child: Text('Data inventory tidak ditemukan'),
                  );
                }
...
class _ProductCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final need = item.minQty <= 0
        ? 0
        : (item.minQty - item.qty).clamp(0, 999999);
                ...
                Wrap(
                  ...
                  children: [
                    ...
                    _iconText(
                      ...
                      '${item.qty} ${item.uom} (min ${item.minQty})',
                    ),
                  ],
                ),

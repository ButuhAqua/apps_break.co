...
class _InventoryBahanPageState extends State<InventoryBahanPage> {
  ...
  String _kategoriFilter = 'Semua'; // Semua / Bahan Baku / Kemasan / Perlengkapan / Operasional
  String _sort = 'Nama (A-Z)'; // Nama (A-Z) / Stok Terendah / Stok Tertinggi / Terbaru

  final List<_StockItem> _data = [
    _StockItem(
      id: 'ST-001',
      name: 'Biji Kopi',
      sku: 'BB-KOPI',
  ...
  String _statusOf(_StockItem e) {
    if (e.qty <= 0) return 'Habis';
    if (e.qty <= e.minQty) return 'Low';
    return 'OK';
  }

  List<_StockItem> get _filteredSorted {
    final list = _data.where((e) {
      ...
      final matchCat = _kategoriFilter == 'Semua' || e.category == _kategoriFilter;
  ...
  void _openActions(_StockItem item) {
            ...
            child: Column(
              ...
              children: [
                ...
                _sheetBtn(
                  ...
                  onTap: () {
                    Navigator.pop(context);
                    _snack('Catat pemakaian "${item.name}" (demo)');
                  },
                ),
                const SizedBox(height: 8),
                _sheetBtn(
                  ...
                  onTap: () {
                    Navigator.pop(context);
                    _snack('Mutasi lokasi "${item.name}" (demo)');
                  },
                ),
              ],
            ),
          ...
          Padding(
            ...
            child: TextField(
              ...
              decoration: InputDecoration(
                hintText: 'Cari nama / SKU / kategori…',
            ...
            child: ListView(
              ...
              children: [
                ...
                _segDropdown(
                  ...
                  items: const ['Semua', 'Bahan Baku', 'Kemasan', 'Perlengkapan', 'Operasional'],
                  onChanged: (v) => setState(() => _kategoriFilter = v),
                ),
                const SizedBox(width: 12),
                _sortDropdown(),
              ],
...
class _StockCard extends StatelessWidget {
  const _StockCard({required this.item, required this.status, required this.onTap});
  ...
  Widget build(BuildContext context) {
    ...
    final pct = item.minQty <= 0
        ? 1.0
        : (item.qty / (item.minQty * 2)).clamp(0.0, 1.0); // bar perbandingan (qty vs target 2x min)
